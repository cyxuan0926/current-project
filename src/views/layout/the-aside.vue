<template>
  <div class="aside-container">
    <div class="aside-top">
      <img class="avatar circle" src="/static/images/user2-160x160.jpg" >

      <div class="info">
        <!-- v-if="Number(user.role)"  {{ user.role | role }}-->
        <span class="bold ellipsis">{{ publicUserInfo.tenantName }}</span>
        ...
        <div class="ellipsis">
          ...
          <template v-if="user.role !== '-1'">
            <span class="bold1">{{ publicUserInfo.nickname }}</span>
          </template>
          ...
          >
            <template #reference>
              <span class="bold1 ml-10">{{ prisonerAreas[0] }}</span>
            </template>
          </el-popover>
        </div>
      </div>
          ...
          >
            ...
            <template v-for="second in item.children">
              <template v-if="!second.children.length">
                <el-menu-item :key="second.path" :index="second.path">
                  {{ second.name }}
                </el-menu-item>
              </template>
