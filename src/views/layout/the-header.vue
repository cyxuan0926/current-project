<template>
  <div class="header-container">
    ...
    <div :class="[ 'header-right', { 'header-right-no__regisition': multistageExamineCountConfigs.total } ]">
      <i class="iconfont icon-category bold" @click="handleCollapse" />
      ...
      <div class="mine__box">
        ...
        <template v-if="isAdvancedAuditor">
          ...
          >
            ...
            <template #dropdown>
              <el-dropdown-menu class="bell-dropdown-menu">
                <template v-for="item in multistageExamineDropdownItems">
                  <template v-if="multistageExamineCountConfigs[item['paramsKey']]">
                    <el-dropdown-item :command="item.path" :key="item.menuName">
                      <span>{{ item.menuName }}</span>

                      <span class="bell__count">
                        {{ +multistageExamineCountConfigs[item['paramsKey']] > 99 ? '99+' : multistageExamineCountConfigs[item['paramsKey']] }}
                      </span>
                    </el-dropdown-item>
                  </template>
                </template>
  ...
  methods: {
    ...
    async handleSubmit(val) {
      const mapArray = [
        ...
        'Two',
        'Three'
      ], params = mapArray.map((val,index) => (
        {
          securityQuestionId: this.questionAnswers[val][0],
          answer: this.questionAnswers[val][1],
          ordinal: index
        }
      ))
