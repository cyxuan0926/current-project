...
export default {
  ...
  activated() {
    this.getRemoteSpecialConfigOld({ jailId: this.jailId }).then(res => {
      if (!res) return
  ...
  methods: {
    ...mapActions(['getRemoteSpecialConfigOld', 'addRemoteSpecialConfig', 'updateRemoteSpecialConfig', 'deleteRemoteSpecialConfig']),
    onAdd(config, index) {
