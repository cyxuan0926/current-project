  ...
  <el-row class="row-container" :gutter="0">
    ...
    <el-col :span="24">
      <el-tabs v-model="tabs" type="card">
        <template v-for="(tab, index) in tabsItems">
          <el-tab-pane :key="index" :label="tab.label" :name="tab.name" />
        </template>
      </el-tabs>
      ...
      >
        ...
        <template #content="{ row }">
          <span v-if="!row.content">
            <template v-if="row.status === 'PENDING' && row.isLock === 1"
              >处理中</template
            >
            <template v-else>{{ row.status | applyStatus }}</template>
          </span>

          <el-tooltip v-else :content="row.content" placement="top">
            <span v-if="row.status === 'PENDING' && row.isLock === 1"
              >处理中</span
            >
    ...
    >
      <div v-if="show.agree" class="button-box">
        ...
        <section v-show="!isSpecial">
          ...
          >
            <el-table-column
              v-if="
                meetingAdjustmentCopy.meetingQueue &&
                meetingAdjustmentCopy.meetingQueue.length > 7
              "
              fixed
              prop="terminalNumber"
              label="终端号"
            ...
            <el-table-column
              v-for="(item, index) in meetingAdjustmentCopy.meetingQueue"
              :key="index"
              :prop="item"
              :label="item"
              min-width="84"
      ...
      <div v-if="show.agree" slot="footer" class="dialog-footer">
        <el-button
          ...
          v-if="submitSuccessParams && userDefinedDuration"
          >{{ `选择${!isSpecial ? "其他" : "常规"}时间段` }}</el-button
        >
        <el-button
          type="primary"
          @click="submitSuccess"
          :disabled="!submitSuccessParams"
          >确 定</el-button
        >
    ...
    >
        ...
        <div style="display: flex; border: 1px solid #e4e7ed">
          ...
          <div class="detail-content">
            ...
            <p class="detail-message-family" style="border: none">
              ...
              <span class="family-nameDetail">{{
                toShow.meetingTime || toShow.applicationDate
              }}</span>
            </p>
          </div>
        </div>
      ...
      <template
        v-if="
          isAdvancedAuditor &&
          toAuthorize.changeLogs &&
          Array.isArray(toAuthorize.changeLogs) &&
          toAuthorize.changeLogs.length
        "
      >
        <m-multistage-records
          :values="toAuthorize.changeLogs"
          :keys="multistageExamineKeys"
        />
      </template>
      ...
      <div slot="footer" class="dialog-footer">
        <div class="process-select-block" v-if="!show.agree && !show.disagree">
          <!-- 审批流 -->
          <label
            v-if="show.subTask && show.process"
          ...
          <repetition-el-buttons
            v-if="!show.multistageExamine"
            style="float: right"
            :buttonItems="authorizeButtons"
          />
        </div>
        ...
        <div
          ...
          class="button-box more-button__box"
        >
        ...
        <div v-if="show.disagree" class="button-box logMgCls">
          ...
          <el-button type="danger" plain @click="closeWithdraw('refuseForm')"
            >关闭</el-button
          >
        </div>
      </div>
    ...
    >
      ...
      <el-button plain :loading="btnDisable" @click="onWithdraw('WITHDRAW')"
        >提交</el-button
      >
      <el-button type="danger" plain @click="closeWithdraw('withdrawForm')"
        >关闭</el-button
      >
    </el-dialog>
    <el-dialog
        ...
        >
          ...
          <div class="detail-message">
            <p class="detail-message-family" v-if="item.operateAccount">
              <span class="family-name" v-if="item.status != 'CANCELED'"
                >审核人员账号</span
              >
            ...
            <template v-if="item.meetingCalls">
              <label v-for="(val, keys) in item.meetingCalls" :key="keys">
                ...
                <!-- v-if="isDevelop(val,keys ) -->
                <p class="detail-message-family" v-if="val.remark">
            ...
            <p class="detail-message-family" style="border: none">
              ...
              <span class="family-name" v-if="item.status == 'CANCELED'"
                >取消时间</span
              >
              <span class="family-name" v-if="item.status == 'EXPIRED'"
                >过期时间</span
              >
              <template v-if="item.status == 'CALL'">
                <span class="family-name" v-if="item.callType == 0"
                  >呼叫时间</span
                >
                <template v-if="item.callType == 1">
                  <span class="family-name" v-if="item.updatedAt"
                    >系统电话通话时间</span
                  >
              ...
              <template v-else>
                <span
                  class="family-nameDetail"
                  v-if="!item.meetingCalls || item.status == 'EXPIRED'"
            ...
            <p
              v-if="
                item.status == 'CALL' && item.updatedAt && item.callType == '1'
              "
            ...
            <p
              ...
              v-if="item.remark"
            >
              <span class="family-name">备注</span>
          ...
          <div class="detail-content">
            ...
            <p class="detail-message-family">
              <template v-if="item.status !== 'CALL'">
                ...
                <span class="family-nameDetail" v-if="item.status == 'PASSED'"
                  >已通过</span
                >

                <span class="family-nameDetail" v-if="item.status == 'CANCELED'"
                  >已取消</span
                >

                <span class="family-nameDetail" v-if="item.status == 'DENIED'"
                  >已拒绝</span
                >

                <span class="family-nameDetail" v-if="item.status == 'EXPIRED'"
                  >已过期</span
                >
                <span class="family-nameDetail" v-if="item.status == 'ENDED'"
                  >已结束</span
                >

                <span class="family-nameDetail" v-if="item.status == 'FINISHED'"
                  >已完成</span
                >

                <span
                  class="family-nameDetail"
                  v-if="item.status == 'MEETING_ON'"
                  >通话中</span
                >
                <span class="family-nameDetail" v-if="item.status == 'SUBMIT'"
                  >已提交二级审核</span
                >
              </template>
              <template v-if="item.status == 'CALL'">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["isShowPhone", "isSuperAdmin"]),

    excelDownloadUrl() {
      return this.hasAllPrisonQueryAuth || this.hasProvinceQueryAuth
        ? "/download/exportMettings"
        : "/download/exportMettingsJail";
    },
    ...
    excelFilter() {
      const tabItem = this.tabsItems.filter(
        (tabItem) => tabItem.name === this.tabs
      );
    ...
    tableCols() {
      ...
      if (!["first"].includes(this.tabs))
        helper.arrayRemove(basicCols, "终端唯一标识", "label");
      ...
      if (this.hasAllPrisonQueryAuth || this.hasProvinceQueryAuth) {
        ...
        return [...basicCols, ...noAllPrisonQueryAuthLeadingCols];
      }
    },
  },
    ...
    meetingRefresh(val) {
      if (val) {
        if (
          ...
          !this.show.withdraw &&
          !this.toShow.id &&
          !this.show.familiesDetialInform
        )
  ...
  created() {
    this.searchItems.area.options = JSON.parse(localStorage.getItem("user"))
      .separateByArea
      ? this.$store.state.areaOptions
      : this.AreaObj;
  },
  ...
  methods: {
          ...
          .then((res) => {
            ...
            if (this.tabs === "UNUSUAL") {
              ...
              if (this.hasAllPrisonQueryAuth)
                url = "/meetings/findUnusualAdminPage";
    ...
    setSelectRange() {
      ...
      this.selectRange = {
        selectableRange: `${
          this.toAuthorize &&
          this.toAuthorize.applicationDate === Moment(_now).format("YYYY-MM-DD")
            ? Moment(_now).format("HH:mm")
            : "00:00"
        }:00 - 23:58:00`,
        format: "HH:mm",
      };
    ...
    getMeetTimeConfig() {
      http
        .getMeetTimeConfig({
          id: this.getMeetingId,
          area:
            this.isSeparateByArea || this.isUseMeetingFloor
              ? this.areaTabs
              : "",
        })
        .then((res) => {
          ...
          this.show.meetingQueue =
            this.meetingAdjustment.meetingQueue.length > 0 ? false : true;
          this.meetingAdjustmentCopy = JSON.parse(
            JSON.stringify(this.meetingAdjustment)
          );
          ...
          this.durationOptions =
            this.meetingAdjustment.meetingChargeTemplates &&
            this.meetingAdjustment.meetingChargeTemplates.map((c) => {
              return {
                label: c.duration,
                value: c.duration,
              };
    ...
    checkInmeetings() {
      return this.meetingAdjustmentCopy.meetingQueue.some((m) => {
        ...
        return (
          (this.timeRangeStart.diff(sm) > 0 &&
            this.timeRangeStart.diff(em) < 0) ||
          (this.timeRangeEnd.diff(sm) > 0 && this.timeRangeEnd.diff(em) < 0)
        );
      });
    },
    ...
    async getSubtask({ processInstanceId }) {
      ...
      this.nextCheckCode = !this.selectProcessOption.length
        ? ""
        : this.selectProcessOption[0].taskCode;
    },
      ...
      this.$nextTick(() => {
        if (this.$refs[`itemHeigh+${key}`]) {
          this.$set(
            val,
            "itemHeigh",
            this.$refs[`itemHeigh+${key}`][0].clientHeight
          );
        }
      });
      return val.remark;
    ...
    onAgreeAuthorize() {
      ...
      if (
        !this.toShow.processInstanceId ||
        (this.toShow.isChoiceTime && !this.show.subTask)
      ) {
        this.show.agree = true;
        this.buttonLoading = false;
      } else {
        let { id, terminalId, meetingTime, processInstanceId, isChoiceTime } =
          this.toShow;
        this.submitParams = {
          meetingId: id,
      ...
      if (e === "DENIED" || e === "WITHDRAW") {
        if (e === "DENIED") {
          this.$refs.refuseForm.validate((valid) => {
            ...
            if (valid)
              params.remarks = this.refuseForm.anotherRemarks.replace(
                /\s*/g,
                ""
              );
    ...
    submitSuccess: function () {
      ...
      if (
        this.compareDate(
          new Date(),
          `${
            this.toAuthorize.meetingTime || this.toAuthorize.applicationDate
          } ` + this.submitSuccessParams.meetingTime.toString().substring(0, 5)
        )
      ) {
        this.$message.closeAll();
        this.$message({
        ...
        // 是否分监舍区生产区或者会见楼
        if (this.isSeparateByArea || this.isUseMeetingFloor) {
          this.submitParams.area = this.isSpecial
            ? this.areaTypes
            : this.areaTabs;
        }
        // 是否选择特殊时间段
        if (this.isSpecial) {
          if (this.checkInmeetings()) {
            this.showTips =
              "（通话时间与常规配置中的通话时间冲突，请重新选择！）";
            this.isShowTips = true;
            return;
          }
          this.submitParams.meetingTime = `${Moment(this.timeRangeStart).format(
            "HH:mm"
          )}-${Moment(this.timeRangeEnd).format("HH:mm")}`;
        }
        // 非审批流 选择完时间则直接提交审核
    ...
    async submitMeetingAuthorize() {
      ...
      let res = await http[
        this.isSpecial
          ? "meetingSelectOtherAuthorize"
          : "meetingSelectAuthorize"
      ](this.submitParams);
      if (res) {
        this.closeAuthorize();
        this.toAuthorize = {};
      ...
      this.getMeetingsFamilyDetail({ meetingId, familyId }).then((res) => {
        if (res.family) {
          ...
          for (let [key, value] of Object.entries(res.family)) {
            ...
            keys.includes(key) &&
              value &&
              res.family.relationalProofUrls.push({
                url: value,
              });
          }
