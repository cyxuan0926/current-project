  ...
  <el-row class="row-container" :gutter="0">
    ...
    <el-col :span="24">
      ...
      >
        ...
        <el-table-column prop="phone" min-width="90" label="家属电话">
          <template #default="{ row }">
            <el-popover
              popper-class="is-asterisk_display"
              placement="top-start"
            ...
            >
              <span slot="reference">{{
                row.phone | asteriskDisplay("asterisk_phone")
              }}</span>
            </el-popover>
          </template>
        </el-table-column>
        ...
        >
          <template #default="{ row }">
            ...
            >
              <span slot="reference">{{
                row.prisonerName | asteriskDisplay("asterisk_name")
              }}</span>
            </el-popover>
          </template>
        </el-table-column>

        <el-table-column label="罪犯编号" prop="prisonerNumber" min-width="50">
          <template #default="{ row }">
            <el-popover
              popper-class="is-asterisk_display"
              placement="top-start"
            ...
            >
              <span slot="reference">{{
                row.prisonerNumber | asteriskDisplay("asterisk_prisonerNumber")
              }}</span>
            </el-popover>
          </template>
        </el-table-column>
        ...
        <el-table-column label="申请状态" min-width="60" class-name="orange">
          <template #default="{ row }">
            {{ row.status | registStatus }}
          </template>
        </el-table-column>
            ...
            <template v-if="!hasAllPrisonQueryAuth">
              ...
              <template v-if="!row.processInstanceId">
                ...
                <template v-if="row.status === 'PASSED'">
                  ...
                  <el-button size="mini" @click="onDownload(row)"
                    >下载</el-button
                  >
                </template>
    ...
    >
      ...
      <div class="button-box view-box">
        <el-button type="danger" plain @click="showDetail = false"
          >关闭</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      ...
      <div
        ...
        :style="{
          'margin-bottom':
            (toAuthorize.relationalProofUrls &&
              toAuthorize.relationalProofUrls.length) ||
            toAuthorize.meetNoticeUrl
              ? '10px'
              : '40px',
        }"
      >
      ...
      >
        ...
        <div class="img-box">
          <template
            v-for="relationalProofUrl in toAuthorize.relationalProofUrls"
          >
            <m-img-viewer
              ...
              :toolbar="
                toAuthorize.relationalProofUrls.length === 1
                  ? {}
                  : { prev: 1, next: 1 }
              "
              title="关系证明图"
      ...
      <template v-if="!hasAllPrisonQueryAuth">
        ...
        <div
          ...
          class="button-box multistage-examine_button_box el-row_callback"
        >
          ...
          <el-button type="danger" plain @click="onFirstLevelClose"
            >关闭</el-button
          >
        </div>
            ...
            <div v-else class="button-box">
              ...
              >
                <template #nextCheckCodeAgreeButtons>
                  <el-select
                    v-model="agreeHasSubTaskFormData.nextCheckCode"
                    placeholder="请选择审核人"
                  >
                  ...
                  <el-row class="el-form__buttons">
                    ...
                    <el-button plain @click="show.agree = false"
                      >返回</el-button
                    >
        ...
        <div v-if="show.disagree" class="button-box logMgCls el-row_callback">
          ...
          <div class="disagree__div">
            ...
            <el-button type="primary" @click="onRejectshow('PASSED')"
              >编辑驳回原因</el-button
            >
          </div>
        ...
        <div v-if="show.callback" class="button-box logMgCls el-row_callback">
          ...
          <el-button plain :loading="buttonLoading" @click="submitReject"
            >提交</el-button
          >
    ...
    >
      <div class="flex-dialog">
        <div style="width: 100%">
          <label>家属姓名：</label><span>{{ notification.familyName }}</span>
        </div>

        <div style="width: 100%">
          <label>家属身份证号：</label
          ><span>{{ notification.familyUuid }}</span>
        </div>

        <div style="width: 100%">
          <label>与服刑人员关系：</label
          ><span>{{ notification.familyRelationship }}</span>
        </div>

        <div style="width: 100%">
          <label>协议编号：</label><span>{{ notification.protoNum }}</span>
        </div>

        <div style="width: 100%">
          <label>签署日期：</label><span>{{ notification.signDate }}</span>
        </div>
...
export default {
  ...
  watch: {
    tabs(val) {
      ...
      if (!_res) {
        this.searchItems.applicationDate.value = [
          this.$_oneMonthAgo,
          this.$_dateNow,
        ];
  ...
  computed: {
    ...
    ...mapState({
      processInstanceIdSubtaskOptions: (state) =>
        state.global.processInstanceIdSubtaskOptions,
      currentProcessTaskInformation: (state) =>
        state.global.currentProcessTaskInformation,
    }),
    ...
    isSubtask() {
      return !!(
        this.processInstanceIdSubtaskOptions &&
        Array.isArray(this.processInstanceIdSubtaskOptions) &&
        this.processInstanceIdSubtaskOptions.length
      );
    },
  ...
  methods: {
    ...
    async handleExportExcel() {
      ...
      try {
        ...
        saveAs(
          data,
          `家属注册列表-${DateFormat(Date.now(), "YYYYMMDDHHmmss")}.xls`
        );
    ...
    async onGetRegistrationDetail(row) {
      const { id, name, relationship, prisonerName, remarks } = row;
      ...
      if (e === "DENIED" || e === "WITHDRAW") {
        ...
        if (e === "WITHDRAW") {
          ...
          this.$refs.withdrawForm.validate((valid) => {
            ...
            if (valid)
              params.withdrawReason = this.withdrawForm.withdrawReason.replace(
                /\s*/g,
                ""
              );
            else this.buttonLoading = false;
    ...
    set_relationalProofUrls(authorizeDetData) {
      if (authorizeDetData) {
        ...
        for (let index = 0; index < 4; index++) {
          if (index === 0 && authorizeDetData.relationalProofUrl)
            _relationalProofUrls.push({
              url: authorizeDetData.relationalProofUrl,
            });
          else {
            const num = `relationalProofUrl${index + 1}`;
            authorizeDetData[num] &&
              _relationalProofUrls.push({ url: authorizeDetData[num] });
          }
        }
        ...
        if (
          !authorizeDetData.changeList ||
          !Array.isArray(authorizeDetData.changeList)
        )
          authorizeDetData.changeList = [];
    ...
    closeWithdraw() {
      ...
      this.withdrawForm.withdrawReason =
        this.registrationWithdrawOrAnthorinputReason;
      this.refuseForm.anotherRemarks =
        this.registrationWithdrawOrAnthorinputReason;
      if (this.$refs.refuseForm) this.$refs.refuseForm.clearValidate();
      if (this.$refs.withdrawForm) this.$refs.withdrawForm.clearValidate();
      if (this.$refs.multistageExamineForm)
    ...
    async onDownload(contents) {
      ...
      if (contents === "all") {
        ...
        const passedData = this.registrations.contents.filter(
          (content) => content.status === "PASSED"
        );

        params = this.filterDownloadParams(passedData);

        (type = "zip"), (menuName = `${basicMenuName}${dayNow}`);
      }
    ...
    async onAuthorizeFirstLevel() {
      try {
        ...
        (await this.$refs.multistageExamineForm.validate()) &&
          (this.buttonLoading = true) &&
    ...
    async onAgreeAuthorize() {
      ...
      if (this.isSubtask) {
        ...
        this.$set(
          this.agreeHasSubTaskFormData,
          "nextCheckCode",
          this.processInstanceIdSubtaskOptions[0]["taskCode"] || ""
        );
      }
    ...
    async onPassedAuthorize() {
      ...
      if (this.isSubtask) {
        this.$refs.agreeHasSubTaskForm &&
          this.$refs.agreeHasSubTaskForm.onSubmit();
        ...
        const { taskName = "" } =
          this.processInstanceIdSubtaskOptions.filter(
            (subtask) => subtask.taskCode === nextCheckCode
          )[0] || {};
    ...
    onCloseAuthorize() {
      this.$refs.agreeHasSubTaskForm &&
        this.$refs.agreeHasSubTaskForm.onClearValidate();
