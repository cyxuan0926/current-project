<template>
    <div>
      ...
      <span v-if="toAuthorize.status=='WITHDRAW' ||toAuthorize.status=='DENIED ' ">
        <div style="margin-bottom: 10px;" v-if="toAuthorize.remarks">
          {{ toAuthorize.status === 'WITHDRAW' ? '撤回原因：' : '审核未通过原因：' }}
        </div>
