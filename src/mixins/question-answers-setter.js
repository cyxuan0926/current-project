...
export default {
  data() {
    let formButton =
      {
        buttons: [ { add: 'add', text: '确定' }, 'cancel' ]
      }, inputItem =
      {
        type: 'input',
        label: '答案',
  ...
  methods: {
    ...
    onProcesseData(data) {
      const {
        ...
        answerTwo,
        aneswrThree
      } = data, One = [questionOne, answerOne], Two = [questionTwo, answerTwo], Three = [questionThree, aneswrThree]
    ...
    async onRenderData(answers) {
      ...
      if (answers && answers.length) {
        const questionAnswers = answers.map(val => [ val.securityQuestionId, val.answer ]),
          noOne = [questionAnswers[1][0], questionAnswers[2][0]],
          noTwo = [questionAnswers[0][0], questionAnswers[2][0]],
          noThree = [questionAnswers[0][0], questionAnswers[1][0]]
        ...
        this.values = {
          questionOne: questionAnswers[0][0],
        ...
        this.fields.map(val => this.$set(this.questionAnswersFormItems[val], 'options', this.securityQuestions))
      }
    }
  },
